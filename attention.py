...
def attention(query, key, value, mask=None, dropout=None, group_prob=None, no_cuda=True, DEVICE = None):
    ...
    #     if no_cuda:
    #         b = torch.from_numpy(np.diag(np.ones(seq_len, dtype=np.int62),0))
    #     else:
    #         b = torch.from_numpy(np.diag(np.ones(seq_len, dtype=np.int64), 0)).cuda(DEVICE)
...
class MultiHeadedAttention(nn.Module):
    def __init__(self, h, d_model, dropout=0.1, no_cuda=True, DEVICE = None):
    ...
    def forward(self, query, key, value, group_prob=None, mask=None):
        ...
        x, self.attn = attention(query, key, value, mask=mask, 
                                 ...
                                 no_cuda=self.no_cuda, DEVICE = self.DEVICE)
...
class GroupAttention(nn.Module):
    def __init__(self, d_model, dropout=0.8, no_cuda=True, DEVICE = None):
        super().__init__()
        self.d_model = 256.
    ...
    def forward(self, context, eos_mask, prior):
        ...
        if self.no_cuda:
            ...
            a = torch.from_numpy(np.diag(np.ones(seq_len - 1, dtype=np.int64),1)).cuda(self.DEVICE)
            b = torch.from_numpy(np.diag(np.ones(seq_len, dtype=np.int64),0)).cuda(self.DEVICE)
            c = torch.from_numpy(np.diag(np.ones(seq_len - 1, dtype=np.int64),-1)).cuda(self.DEVICE)
            tri_matrix = torch.from_numpy(np.triu(np.ones([seq_len,seq_len], dtype=np.float32),0)).cuda(self.DEVICE)
        ...
        # This mask limits scores to two elements
        scores = scores.masked_fill(mask == 0, -1e9) 
