...
# -*- coding: utf-8 -*-
"""
Created on Sat Jun  4 08:42:56 2022

@author: valancewang


Transformer and Tree Transformer Evaluate on Newsela - Testset 400
- Newsela dataset
- Add test
...
if CUDA:
    ...
    model_path = '/ssddata1/data/valance/AI/Transformer/'
else:
...
if CUDA:
    model.load_state_dict(
        torch.load(
        model_path + "Transformer_L4H5_Newsela_Zhang_max_200_211_checkpoint.pk1",
...
# Define path and load data
if CUDA:
    data_path = '/ssddata1/data/valance/AI/data/text_simplification/newsela_article_corpus_2016-01-29/articles/'
    model_path = '/ssddata1/data/valance/AI/Tree-Transformer/train_model/'
else:
    data_path = '/Users/valancewang/Dropbox/AI/data/text_simplification/newsela_article_corpus_2016-01-29/articles/'
    model_path = '/Users/valancewang/Dropbox/AI/Models/Tree Transformer/'
...
if CUDA:
    tree_model.load_state_dict(torch.load(
        model_path + "newsela_zhang_max_45_checkpoint.pth")['model_state_dict'])
else:
    tree_model.load_state_dict(torch.load(
        model_path + "newsela_zhang_max_45_checkpoint.pth", 
        map_location=torch.device('cpu'))['model_state_dict'])
...
# Evaluate
def evaluate(model, sentence, device = None, max_length = MAX_LENGTH):    
...
def evaluate_idx_test(model, tree_model, idx, c):    
    print("Test set: ", idx)  
    ...
    output_words = evaluate(model, src, device = device_t)
    output_sentence = ' '.join(output_words)
    output_words_tree = evaluate(tree_model, src, device = device_tt)
    output_sentence_tree = ' '.join(output_words_tree)        
    # Write log_file
    f = open('./tree_newsela-400.txt','a')
...
def evaluate_400(model, tree_model):
    ...
    c = 0 
    for idx in idx_list:
        evaluate_idx_test(model, tree_model, idx, c)
        c += 1        
    return


evaluate_400(model, tree_model)
