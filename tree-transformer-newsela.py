...
# -*- coding: utf-8 -*-
"""
Created on Wed Mar 23 13:31:37 2022
...
- Add GloVe embedding
- Adapt to Newsela dataset
- Use Zhang and Lapata's partition
...
# # # Load model
# # Load model
# model_name = "NAT_L10H5_small_epoch_1500_checkpoint.pth"
# print(model_name)
# if CUDA:
...
step = 0
for epoch in range(OFFSET+1, OFFSET+N_ITERATION+1):
    ...
    for idx_epoch, (input_str, target_str) in enumerate(train_loader):
        ...
        if step % 200 == 1:
            ...
            # Write log_file
            f = open('./log_file_newsela_zhang_max_500.txt','a')
        ...
        if step % 1000 == 0:
            ...
            model_name = 'newsela_zhang_max_' + str(epoch) + '_checkpoint.pth'
            torch.save({
                    'epoch': epoch,
            ...
            bleu_valid = evaluate_all_valid(model)

            # Write log_file            
            f = open('./log_file_newsela_zhang_max_BLEU_500.txt','a')
            f.write("iteration:%s, Epoch Step: %d, BLEU:%.2f" % 
                      (epoch, step, bleu_valid ))
            f.write('\n')
            f.close()
