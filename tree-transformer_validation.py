...
# -*- coding: utf-8 -*-
"""
Created on Sun Jan 30 14:48:18 2022
...
# -----------------------------------------------------------------------
# Load validation set
fname_src = 'wiki.full.aner.valid.src'
fname_dst = 'wiki.full.aner.valid.dst'

# Load the validation set
data_valid_src = []
data_valid_dst = []
data_valid_src += open(path + fname_src, "r", encoding="utf-8").read().split('\n')
data_valid_dst += open(path + fname_dst, "r", encoding="utf-8").read().split('\n')
...
if CUDA:
    model.load_state_dict(torch.load(
        model_path + "ts_L10H5_length200_epoch_404.pth")['state_dict'])
else:
    model.load_state_dict(torch.load(
        model_path + "ts_L10H5_length200_epoch_404.pth", 
...
def evaluateRandomly_valid(model, n=3):
    ...
    for i in range(n):
        ...
        print(">", data_valid_src[idx])
        print("=", data_valid_dst[idx])
        print(">", ' '.join(src_unknown))
        print("=", ' '.join(dst_unknown))
        
        output_words = evaluate(model, data_valid_src[idx])
...
def evaluate_all_valid(model):
    ...
    for idx in range(len(data_valid_src)):
    ...
    bleu_total = bleu_total / len(data_valid_src)
    sari_total = sari_total / len(data_valid_src)
